"""Reproducible named endpoints for illustrative HF route scenarios."""
...
def location_names() -> list[str]:
    """Return alphabetically sorted display names for searchable selectors."""
    return sorted(HF_LOCATIONS)


def resolve_route_scenario(name: str) -> tuple[dict, dict]:
    """Return independent endpoint dictionaries for one named scenario."""
...
def resolve_location(name: str) -> dict:
    """Return an independent endpoint dictionary for one catalogue location."""
